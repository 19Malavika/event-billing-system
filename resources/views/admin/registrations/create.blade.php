<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('registrations.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="event_id" id="event_id" x-model="selectedEventId" @change="updateEventFee" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            ...
                            @foreach($events as $event)
                                <option value="{{ $event->id }}" data-fee="{{ $event->registration_fee }}" data-seats="{{ $event->available_seats }}">
                                    {{ $event->title }} (Fee: ₹{{ $event->registration_fee }} | Seats Left: {{ $event->available_seats }})
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Volume Discount: 3-4 tickets (5% off), 5+ tickets (10% off)</p>
                    </div>

                    <!-- Registration Date -->
                    <div>
                        <label for="registration_date" class="block text-sm font-medium text-gray-700">Registration Date</label>
                        <input type="datetime-local" name="registration_date" id="registration_date" value="{{ now()->format('Y-m-d\TH:i') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    </div>

                    <!-- Additional Options -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <!-- Additional Workshop (₹200 per ticket) -->
                        ...
                        <div>
                            ...
                            <select name="food_preference" id="food_preference" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div class="flex justify-end space-x-3">
                        ...
                        <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-md text-sm hover:bg-indigo-700">Complete Registration</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <!-- Alpine.js Live Calculation Script -->
    <script>
